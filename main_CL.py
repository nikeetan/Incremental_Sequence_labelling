...
def main_cl(params):
    """
    Main function for continual learning (CL) experiment.
    Args:
        params (Namespace): Parameters for the experiment.
    Initializes the Accelerator, sets random seeds, initializes the experiment logger, and sets up wandb tracking.
    Retries wandb connection up to 5 times in case of connection errors or timeouts.
    Loads the dataset and runs the inner training loop with an executable batch size.
    Comments:
